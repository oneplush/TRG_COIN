...
// File: contracts/TRGCoin.sol

/*
Intreprind IQ developed TRG COIN as a Hyper-deflationary token that rewards investors for holding tokens while also
implementing state of the art Buy Back mechanism
...
contract TRGCOIN is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable private IQAddress;
    ...
    constructor (string memory _n, string memory _s, uint8 _d, uint256 _ts, uint256 _tax, uint256 _bb, uint256 _mkt, address _ma) {
        ...
        require(_mkt >= 15, 'Marketing fee can not be less than 1.5%');
        ...
        IQAddress = payable(0xE87805cc8cEAAAE1A3970E88171257A25B971e08);
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(IQAddress, transferredBalance.div(_liquidityFee).mul(IQFee));
        transferToAddressETH(marketingAddress, transferredBalance.div(_liquidityFee).mul(marketingFee.sub(IQFee)));
    ...
    function setMarketingFee(uint256 _marketingFee) external onlyOwner() {
        require(_marketingFee >= IQFee, 'Marketing fee can not be less than 5 percent');
